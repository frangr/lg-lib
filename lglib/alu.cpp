...
ALU::ALU(int li):l(li), rs(new ldr::bit[l]) {} //li = lunghezza input

ALU::~ALU() {delete [] rs;}

void ALU::set_op(ldr::bit ope1, ldr::bit ope2)
{
    op1 = ope1;
    op2 = ope2;
}
...
ldr::bit ALU::r_res(int idx) const {return rs[idx];}

ldr::bit ALU::getflag(int idx) const {return flag[idx];}

void ALU::add(ldr::bit bf[]) //nci = numero cicli
{
    /*
...
void ALU::uadd(ldr::bit bf[]) //nci = numero cicli
{
    /*
    se A=1, B=1, carry = 0, allora rs = 0, carry = 1
    se A != B, carry = 1, allora rs = 0, carry = 1
